...
def train(all_data):
    ...
    # 也可以使用一下看看效果
    train_data = all_data[(all_data.day != 29) & (all_data.day != 1)]
    ...
    for i, date in enumerate(slip):
        ...
        cat_model_out = out_cat_model

        print("第%d轮完成"%(i+1))
    print("全部结束！")

    cat_model_in.save_model(fname="cat_model_in.json",format="json")
    cat_model_out.save_model(fname="cat_model_out.json",format="json")
    ## 搭建LGB模型
    # 设置模型参数
    lgb_params = {
        'boosting_type': 'gbdt',
    ...
    callbacks = [log_evaluation(period=1000), early_stopping(stopping_rounds=200)]
    ## 预测进站流量
    in_lgb_pred = np.zeros(len(X_test))
    X_data = train_data[features].values
    y_data = train_data['inNums'].values
    lgb_model_in = lgb.Booster
    lgb_model_out = lgb.Booster
    cat_model_in = catboost.CatBoost
    cat_model_out  = catboost.CatBoost
    for i, date in enumerate(slip):
        train = train_data[train_data.day<date]
        valid = train_data[train_data.day==date]
        X_train = train[features].values
    ...
    for i, date in enumerate(slip):
        ...
        print("\n Fold ", i,len(X_train),len(y_train),len(X_eval),len(y_eval))
        lgb_train = lgb.Dataset(X_train, y_train)
        lgb_eval = lgb.Dataset(X_eval, y_eval, reference=lgb_train)
        ...
        print("best rounds: ",lgb_model.best_iteration)
        # 用所有的数据训练再预测
        all_train = lgb.Dataset(X_data, y_data)
        ...
        lgb_model_out  = lgb_model_out
        out_lgb_pred += lgb_model_out.predict(X_test) / n
        print("第%d轮训练结束"%(i+1))
    print("完成！")
    lgb_model_in.save_model('lgb_model_in.txt')
    lgb_model_out.save_model('lgb_model_out.txt')
